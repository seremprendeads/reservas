-- ============================================================================
-- Plan 4 (bio_web · "Bio Pro + Sitio web") ahora incluye el modulo Reservas.
--
-- Cambio pedido explicitamente: SOLO el plan bio_web. No se toca ningun otro
-- plan (bio_reservas_web, bio_web_shop, enterprise, etc.).
--
-- Nota: a partir de esta migracion, bio_web (bio, landing, reservas) y
-- bio_reservas_web (bio, reservas, landing) otorgan exactamente los mismos
-- modulos.
-- ============================================================================
  ...
  SELECT EXISTS (
    ...
    WHERE b.id = p_business_id
      ...
      AND (
        ...
        OR (b.is_trial = false AND b.plan = 'bio_web' AND p_module IN ('bio', 'landing', 'reservas'))
