...
export function BioAdmin({ adminEmail }: { adminEmail: string }) {
    ...
    setProfile(prev => {
      if (!prev) {
        const base: BioProfile = {
          ...
          bg_opacity: 0, bg_overlay_color: '#000000',
  ...
  const saveDraft = async () => {
    ...
    const toSave = Object.keys(fields).length > 0 ? {
      ...
      bg_gradient_to: p.bg_gradient_to, bg_image_url: p.bg_image_url, bg_opacity: p.bg_opacity, bg_overlay_color: p.bg_overlay_color,
          ...
          {tab === 'appearance' && (
            <Card className="border-border/50">
              <CardContent className="p-6 space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Capa de color sobre el fondo</label>
                  <div className="flex items-center gap-3">
                    <input type="color"
                      value={profile?.bg_overlay_color || '#000000'}
                      onChange={e => { handleFieldChange('bg_overlay_color', e.target.value); debouncedSave(); }}
                      className="h-10 w-10 rounded-lg border cursor-pointer" />
                    <Button variant="outline" size="sm"
                      onClick={() => { handleFieldChange('bg_overlay_color', '#000000'); debouncedSave(); }}>
                      Negro
                    </Button>
                    <Button variant="outline" size="sm"
                      onClick={() => { handleFieldChange('bg_overlay_color', '#ffffff'); debouncedSave(); }}>
                      Blanco
                    </Button>
                  </div>
                  <label className="text-sm font-medium block pt-2">Intensidad</label>
                  <div className="flex items-center gap-3">
                    {/* Sin appearance-none: en Chrome eso borraba el control
                        deslizante y quedaba una barra que no se podia arrastrar. */}
                  ...
                  <p className="text-xs text-muted-foreground">En 0% se ve el fondo limpio. Subí la intensidad para teñirlo y que el texto se lea mejor.</p>
                </div>
                <div className="space-y-2">
              ...
              <div className="rounded-[2.5rem] border-[6px] border-gray-900 bg-gray-900 overflow-hidden shadow-2xl">
                <div className="rounded-[2rem] overflow-hidden relative" style={{
                  ...
                  {(profile?.bg_opacity ?? 0) > 0 && (
                    <div className="absolute inset-0" style={{ backgroundColor: profile?.bg_overlay_color || '#000000', opacity: (profile?.bg_opacity ?? 0) / 100 }} />
                  )}
...
function AddLinkDialog({ open, onClose, onAdd }: { open: boolean; onClose: () => void; onAdd: (title: string, url: string, icon: string) => void }) {
  ...
  // El selector de icono se saco de la interfaz: confundia al cliente y la
  // pagina publica nunca lo dibuja. Se sigue mandando 'link' para no cambiar la
  // firma de addLink ni dejar la columna en null.
  const handleSave = () => { if (title.trim() && url.trim()) { onAdd(title.trim(), url.trim(), 'link'); setTitle(''); setUrl(''); } };
  return (
...
function EditLinkDialog({ link, onClose, onSave }: { link: BioLink; onClose: () => void; onSave: (fields: Partial<BioLink>) => void }) {
  ...
  const [icon] = useState(link.icon || 'link');
